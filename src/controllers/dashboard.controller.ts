...
export const getActiveProducts = async (req: Request, res: Response) => {
  try {
    ...
    if (userRole === 'admin') {
      // Admin can see all active products
    ...
    if (userRole === 'admin') {
      ...
      whereCondition = {
        ...
        [Op.or]: [
          ...
          { promoterId: { [Op.in]: adminUserIds } }  // Admin-created promotions
        ]
      };
    }
...
export const getPromotionStatsByMonth = async (req: Request, res: Response) => {
  try {
    ...
    // Get totals
    const totalClicks = totalStats ? parseInt(totalStats.totalClicks) || 0 : 0;
    const totalConversions = totalStats ? parseInt(totalStats.totalConversions) || 0 : 0;
    const totalEarnings = totalStats ? parseFloat(totalStats.totalEarnings) || 0 : 0;
    
    // Get previous month totals
    const prevMonthClicks = prevMonthStats ? parseInt(prevMonthStats.clicks) || 0 : 0;
    const prevMonthConversions = prevMonthStats ? parseInt(prevMonthStats.conversions) || 0 : 0;
    const prevMonthEarnings = prevMonthStats ? parseFloat(prevMonthStats.earnings) || 0 : 0;
    
    // Calculate percentage changes
    const clicksPercentChange = prevMonthClicks > 0 
      ? ((totalClicks - prevMonthClicks) / prevMonthClicks * 100).toFixed(2).toString()
      : totalClicks > 0 ? '100' : '0';
    
    const conversionsPercentChange = prevMonthConversions > 0 
      ? ((totalConversions - prevMonthConversions) / prevMonthConversions * 100).toFixed(2).toString()
      : totalConversions > 0 ? '100' : '0';
    
    const earningsPercentChange = prevMonthEarnings > 0 
      ? ((totalEarnings - prevMonthEarnings) / prevMonthEarnings * 100).toFixed(2).toString()
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          prevMonthEarnings: Math.round(prevMonthEarnings * 100) / 100,
          clicksPercentChange,
...
// Get promotion statistics for a specific month - alternative implementation
export const getPromotionStatsForMonth = async (req: Request, res: Response) => {
  try {
    const { year, month } = req.params;
    ...
    const totalStatsQuery = await sequelize.query(`
      SELECT 
        COUNT(pc."id") as "totalClicks",
        SUM(CASE WHEN pc."isConversion" = true THEN 1 ELSE 0 END) as "totalConversions",
        SUM(pc."earnings") as "totalEarnings"
      ...
      WHERE 
        pc."timestamp" BETWEEN :startDate AND :endDate
        ${userRole === 'admin' ? '' : ' AND (p."promoterId" = :userId OR p."promoterId" IN (:adminUserIds))'}
    `, {
      replacements: { 
        startDate: startDate.toISOString(), 
        endDate: endDate.toISOString(),
    ...
    const totalStatsResults = totalStatsQuery as unknown as Array<{
      totalClicks: string;
      totalConversions: string;
      totalEarnings: string;
    }>;
    
    const totalStats = totalStatsResults[0] || { 
      totalClicks: '0', 
      totalConversions: '0', 
      totalEarnings: '0' 
    };
    ...
    const prevMonthQuery = await sequelize.query(`
      SELECT 
        COUNT(pc."id") as "clicks",
        SUM(CASE WHEN pc."isConversion" = true THEN 1 ELSE 0 END) as "conversions",
        SUM(pc."earnings") as "earnings"
    ...
    // Type assertion for the query result
    const prevMonthResults = prevMonthQuery as unknown as Array<{
      clicks: string;
      conversions: string;
      earnings: string;
    }>;
    
    const prevMonthStats = prevMonthResults[0] || { 
      clicks: '0', 
      conversions: '0', 
      earnings: '0' 
    };
    ...
    // Get totals
    const totalClicks = parseInt(totalStats.totalClicks) || 0;
    const totalConversions = parseInt(totalStats.totalConversions) || 0;
    const totalEarnings = parseFloat(totalStats.totalEarnings) || 0;
    
    // Get previous month totals
    const prevMonthClicks = parseInt(prevMonthStats.clicks) || 0;
    const prevMonthConversions = parseInt(prevMonthStats.conversions) || 0;
    const prevMonthEarnings = parseFloat(prevMonthStats.earnings) || 0;
